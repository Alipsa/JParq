...
A simple fix is not possible. A significant refactoring is required to introduce a robust and consistent identifier handling mechanism. This would involve creating a new Identifier class to store the original identifier and its quoted
status, and then refactoring all the affected files to use this new class.

Create a plan to fix the inconsistent Post-Parse Handling for quoted identifiers: While the parser preserves case, downstream processing can be inconsistent. For instance, Common Table
