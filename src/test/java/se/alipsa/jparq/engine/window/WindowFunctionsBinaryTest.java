...
class WindowFunctionsBinaryTest {
  ...
  void compareBinaryNullHandling() {
    ...
    Assertions.assertTrue(invokeCompare(null, new byte[] {1}) < 0, "Null should be treated as less than non-null");
    Assertions.assertTrue(invokeCompare(new byte[] {1}, null) > 0, "Non-null should be greater than null");
  }

  @Test
  void compareBinaryLexicographicOrdering() {
    Assertions.assertTrue(invokeCompare(new byte[] {1, 2, 3}, new byte[] {1, 2, 4}) < 0,
        "Comparison should defer to the first differing byte");
    Assertions.assertTrue(invokeCompare(new byte[] {1, 2, 4}, new byte[] {1, 2, 3}) > 0,
        "Comparison should defer to the first differing byte");
  }

  @Test
  void compareBinaryLengthWhenPrefixesMatch() {
    Assertions.assertTrue(invokeCompare(new byte[] {1, 2}, new byte[] {1, 2, 0}) < 0,
        "Shorter prefix should be ordered before longer value when content matches");
    Assertions.assertTrue(invokeCompare(new byte[] {1, 2, 0}, new byte[] {1, 2}) > 0,
        "Longer value should be ordered after matching shorter prefix");
  }
