...
public final class JParqUtil {
  ...
  public static ResultSet listResultSet(String[] headers, List<Object[]> rows) {
    return new ResultSet() {
      int idx = -1;
      @Override public boolean next() { idx++; return idx < rows.size(); }
      @Override public void close() {}
      @Override public String getString(String columnLabel) { return String.valueOf(getByLabel(columnLabel)); }
      @Override public String getString(int columnIndex) { return String.valueOf(rows.get(idx)[columnIndex-1]); }
      @Override public Object getObject(String columnLabel) { return getByLabel(columnLabel); }
      private Object getByLabel(String label) {
        for (int i=0;i<headers.length;i++) if (headers[i].equalsIgnoreCase(label)) return rows.get(idx)[i];
        return null;
      }
      ...
      @Override public int findColumn(String columnLabel){ for (int i=0;i<headers.length;i++) if (headers[i].equalsIgnoreCase(columnLabel)) return i+1; return -1; }
      ...
      @Override public java.sql.Date getDate(int columnIndex){return null;} @Override public java.sql.Time getTime(int columnIndex){return null;} @Override public java.sql.Timestamp getTimestamp(int columnIndex){return null;}
