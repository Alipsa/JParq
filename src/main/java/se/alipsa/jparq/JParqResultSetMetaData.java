...
public class JParqResultSetMetaData implements ResultSetMetaData {
  ...
  public JParqResultSetMetaData(Schema schema, List<String> columns, String tableName) {
    this.schema = schema; this.columns = columns; this.tableName = tableName;
  }
  ...
  @Override public String getTableName(int column) { return tableName; }

  @Override
  public int getColumnType(int column) {
    if (schema == null) return Types.OTHER;
    Schema.Field f = schema.getField(getColumnLabel(column));
    Schema s = f.schema().getType() == Schema.Type.UNION ? f.schema().getTypes().stream().filter(t -> t.getType()!= Schema.Type.NULL).findFirst().orElse(f.schema()) : f.schema();
    return switch (s.getType()) {
      ...
      case LONG -> (s.getLogicalType() instanceof LogicalTypes.TimestampMillis || s.getLogicalType() instanceof LogicalTypes.TimestampMicros ? Types.TIMESTAMP : Types.BIGINT);
  ...
  @Override public String getColumnTypeName(int column) { return java.sql.JDBCType.valueOf(getColumnType(column)).getName(); }
